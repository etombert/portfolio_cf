<?php include 'head.php';?>

		<main class="home animsition" data-animsition-in-class="fade-in" data-animsition-in-duration="1000" data-animsition-out-class="fade-out" data-animsition-out-duration="800">
			<section class="section-1">

				<div class="bloc-text">
					<h2 title="Clara Foggetti is a product designer & creative student at hetic based in paris">CLARA FOGGETTI IS A PRODUCT DESIGNER & CREATIVE STUDENT AT HETIC BASED IN PARIS.</h2>
					<a href=".section-2" class="button scrollTo">Start exploring</a>
				</div>

			</section>

			<section id="section-2" class="section-2">

				<a href="pages/trendee" class="bloc-content animsition-link" data-animsition-out-class="fade-out" data-animsition-out-duration="1000">
					<div class="bloc-img">
						<img src="./src/images/accueil/Trendee.png" alt="Trendee">
					</div>
					<div class="bloc-text">
						<h3 title="PRODUCT DESIGN">PRODUCT DESIGN</h3>
						<p>Trendy and stylish at your fingertips</p>
					</div>
				</a>

				<a href="pages/pocketcare" class="bloc-content animsition-link" data-animsition-out-class="fade-out" data-animsition-out-duration="1000">
					<div class="bloc-img">
						<img src="./src/images/accueil/Pocketcare.png" alt="Pocketcare">
					</div>
					<div class="bloc-text">
						<h3 title="PRODUCT DESIGN">PRODUCT DESIGN</h3>
						<p>The health wallet for all the family</p>
					</div>
				</a>

				<a href="pages/oruro" class="bloc-content animsition-link" data-animsition-out-class="fade-out" data-animsition-out-duration="1000">
					<div class="bloc-img">
						<img src="./src/images/accueil/Oruro.png" alt="Oruro">
					</div>
					<div class="bloc-text">
						<h3 title="UI DESIGN">UI DESIGN</h3>
						<p>The most folkloric event in Bolivia</p>
					</div>
				</a>

				<a href="pages/cafe_arabe" class="bloc-content animsition-link" data-animsition-out-class="fade-out" data-animsition-out-duration="1000">
					<div class="bloc-img">
						<img src="./src/images/accueil/Café-arabe.png" alt="Café arabe">
					</div>
					<div class="bloc-text">
						<h3 title="UI DESIGN">UI DESIGN</h3>
						<p>The coffee served with finesse</p>
					</div>
				</a>

				<a href="pages/mandala" class="bloc-content animsition-link" data-animsition-out-class="fade-out" data-animsition-out-duration="1000">
					<div class="bloc-img">
						<img src="./src/images/accueil/Mandala.png" alt="Mandala">
					</div>
					<div class="bloc-text">
						<h3 title="Creativity">Creativity</h3>
						<p>Life floutishes where light shines</p>
					</div>
				</a>

			</section>

		</main>

<?php include 'footer.php';?>
