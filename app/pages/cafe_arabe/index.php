    <?php include '../../head.php';?>

		<main class="project cafe-arabe">
			<section class="section-1">

                <div class="bloc-container">
                    <div class="bloc-text">
                        <h2>Café arabe</h2>
                        <p class="bloc-paragraph">His ancestral recipe still persists until today. Arab coffee is one of the pillars of Middle Eastern culture. Serving Arab coffee is an important aspect of hospitality in Arab societies that is considered a symbol of generosity. Come discover the history and traditions surrounding this drink.</p>

                        <div class="bloc-info">
                            <div class="bloc-info_content">
                                <h3>Role</h3>
                                <p>Designer UI</p>
                            </div>
                            <div class="bloc-info_content">
                                <h3>Client</h3>
                                <p>UNESCO</p>
                            </div>
                            <div class="bloc-info_content">
                                <h3>Year</h3>
                                <p>2017</p>
                            </div>
                        </div>

                        <div class="bloc-arrow">
                            <div id="scroll-down-container">
                                <div id="arrow-down">
                                    <svg width="10px" height="60px" y="-6.242326819299771"><polygon points="4,0 6,0 6,55 10,55 5,60 0,55 4,55" fill="#000000"></polygon>		</svg>
                                    <svg width="10px" height="60px" y="-71.74539609157986"><polygon points="4,0 6,0 6,55 10,55 5,60 0,55 4,55" fill="#000000"></polygon>		</svg>
                                </div>
                            </div>
                        </div>

                    </div>

                    <div class="bloc-img">
                        <img src="../../src/images/cafe_arabe/IMG-header-Cafe.png" alt="">
                    </div>
                </div>

			</section>

            <section class="section-2">
                <div class="bloc-img">
                    <img src="../../src/images/cafe_arabe/IMG-maquette-Cafe.png" alt="">
                </div>
            </section>

            <section class="section-3">
                <div class="bloc-text">
                    <h3>Share and tag your outfits</h3>
                    <p>You can post your favorite looks after tagging the clothes you wear. Over a period of time, users select the outfits they prefer and reward the authors. In your trend line, always discover the accumulation of the most beautiful looks of the previous days.</p>
                </div>
            </section>

            <section class="section-4 parallax-window" data-parallax="scroll" data-image-src="../../src/images/cafe_arabe/IMG-footer-Cafe.png"></section>

			<section class="section-browse">
				<h2>Browse other projects</h2>

				<div class="bloc-browse_container">

                    <a href="../pocketcare" class="bloc-content pocketcare">
                        <div class="bloc-img">
                            <div class="bloc-img-bg"></div>
                        </div>
                        <div class="bloc-text">
                            <h3>UI DESIGN</h3>
                            <p>The most folkloric event in Bolivia</p>
                        </div>
                    </a>

                    <a href="../trendee" class="bloc-content trendee">
                        <div class="bloc-img">
                            <div class="bloc-img-bg"></div>
                        </div>
                        <div class="bloc-text">
                            <h3>Product Design</h3>
                            <p>The health wallet for all the family</p>
                        </div>
                    </a>

                    <a href="../oruro" class="bloc-content oruro">
                        <div class="bloc-img">
                            <div class="bloc-img-bg"></div>
                        </div>
                        <div class="bloc-text">
                            <h3>UI DESIGN</h3>
                            <p>The coffee served with finesse</p>
                        </div>
                    </a>

                    <a href="../mandala" class="bloc-content mandala">
                        <div class="bloc-img">
                            <div class="bloc-img-bg"></div>
                        </div>
                        <div class="bloc-text">
                            <h3>Creativity</h3>
                            <p>Life floutishes where light shines</p>
                        </div>
                    </a>

				</div>
			</section>

		</main>

    <?php include '../../footer.php';?>